...
function usage(message) {
  ...
  console.error('usage: normalize_claude_session.mjs --input <jsonl> --output <json> [--project-path <path>] [--session-id <id>] [--since <iso>] [--until <iso>]');
  process.exit(1);
}
...
function getTimestamp(record) {
  return record.timestamp || record.message?.timestamp || record.created_at || null;
}

function getSessionId(record) {
  return record.sessionId || record.session_id || record.message?.sessionId || record.uuid || 'unknown-session';
}
...
function redactText(text, counts) {
  const replacements = [
    ...
    { kind: 'private_url', pattern: /https?:\/\/(?:private|internal|[^"'\s/]*\.internal)[^"'\s)]+/g, value: '[REDACTED_PRIVATE_URL]' }
  ];
  let output = text;
...
function actorFor(record) {
  ...
  if (record.type === 'toolUseResult') return 'tool';
  return record.type || 'unknown';
}

function kindFor(record, summary) {
  if (record.isSidechain || record.sidechain) return 'sidechain';
  if (record.type === 'toolUseResult') return 'tool_result';
  if (/tool_use|Bash|Read|Edit|Write/i.test(summary)) return 'tool';
...
function isFailure(summary) {
  return /\b(fail(?:ed|ure)?|error|exitCode.?[1-9]|exit code [1-9]|non-zero|missing artifact|AssertionError|expected .+ to exist)\b/i.test(summary);
}
...
const rows = parseJsonl(await readFile(args.input, 'utf8')).filter(({ record }) => {
  if (args['session-id'] && getSessionId(record) !== args['session-id']) return false;
  if (args['project-path'] && record.cwd && record.cwd !== args['project-path']) return false;
  return inTimeWindow(record, args.since, args.until);
});

const redactionCounts = new Map();
const sessionId = args['session-id'] || rows.find(({ record }) => getSessionId(record) !== 'unknown-session')?.record && getSessionId(rows[0].record);
const timeline = [];
const failures = [];
const userInterventions = [];
const artifacts = new Map();
let taskGoal = 'Unknown Claude Code task';
...
for (const { record, line } of rows) {
  ...
  if (taskGoal === 'Unknown Claude Code task' && actor === 'user' && summary) taskGoal = summary.slice(0, 180);
  const event = {
...
if (timeline.some((event) => event.kind === 'sidechain')) signals.push('sidechain_context');

const packet = {
  traceId: `claude-code:${sessionId || 'unknown-session'}`,
  source: 'claude-code',
