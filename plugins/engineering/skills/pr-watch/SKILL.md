...
- GitHub PRs only. If the repo is GitLab, stop and say so.
...
Use this skill's own bundled helper — do not
...
python3 "${CLAUDE_SKILL_DIR}/scripts/fetch_comments.py" --owner "$OWNER" --repo "$REPO" --number "$PR_NUMBER"
```
...
- This skill is intentionally narrow: it is a repair-and-clear
  loop for PR checks and review threads, not a full PR review.
- Phase B uses the bundled `scripts/fetch_comments.py` for thread state.
  That helper is read-only; pr-watch is the one that performs
...
- A clean automated code review is not the same as a green PR: a reviewer bot can be
  satisfied while CI is still red, or while review threads are still open.
